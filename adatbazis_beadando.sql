...
--Különbözõ telephelyek helyszíneinek táblája 
...
--Oktatók táblája, van benne két idegenkulcs, a helyszínnel és az szgk táblával van összekapcsolva
create table Oktato (
...
--Tanulók táblája, két idegenkulccsal, az oktató és a státusz táblákkal összekapcsolva
--Kezdés dátuma nem lehet a mai dátumnál nagyobb
...
--1. Listázzuk az egyes tanulókat (tanulo_id) a beiratkozás dátuma alapján sorrendbe rendezve. 
...
--9. Jelenítse meg, hogy évente hány tanulója van/volt oktatók szerinti (vezetéknév) bontásban az autósiskolának! 
--A részösszegek legyenek megfelelõen jelölve!

select year(t.Kezdes_datum) as 'Kezdés éve',  
		iif(grouping_id(o.Vezeteknev) = 1, 'Évi tanulószám', o.Vezeteknev) as 'Oktató vezetékneve', 
		count(*) as 'Tanulók száma'
from Oktato o join Tanulok t on o.Oktato_id = t.Oktato_id
group by rollup(year(t.Kezdes_datum), o.Vezeteknev)
